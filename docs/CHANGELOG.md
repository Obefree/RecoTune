...
- **Expo Go на SDK 54 больше не запускает RecoTune (вторая причина синего экрана):** Expo удалил `expo-av` из приложения Expo Go в SDK 54 (в v54-доках `expo-av` без бейджа «Included in Expo Go», у `expo-audio`/`expo-video` он есть). RecoTune держит на `expo-av` всё аудио-ядро (Tuner/Studio/Media/Recorder/Video), а `App.tsx` статически импортит `TunerScreen` → `expo-av/build/ExponentAV.js` делает `requireNativeModule('ExponentAV')` и **бросает при старте**: `native module 'ExponentAV' doesn't exist` — это и есть синий/красный экран после долгой загрузки. Батником не чинится. Десктопный/репо `RecoTune.bat` переписан: честно про SDK 54, инструкция на **dev build** (`build-apk.bat` → `app-debug.apk`, та же live-JS итерация с Metro), `npx expo start -c` на дефолтном порту 8081 без `--offline`. Реальная альтернатива — миграция `expo-av` → `expo-audio`/`expo-video` (есть в Expo Go), задача отдельно. См. [docs/features/2026-06-28-expo-go-sdk54-expo-av-removed.md](./features/2026-06-28-expo-go-sdk54-expo-av-removed.md).
